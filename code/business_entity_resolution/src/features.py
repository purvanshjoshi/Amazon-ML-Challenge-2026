...
FEATURE_NAMES = [
    "tfidf_score",       # Blocking TF-IDF cosine similarity (highest predictive gain, 0 compute cost)
    "jw_name",           # Jaro-Winkler similarity on normalized name
    "tsort_name",        # Token Sort Ratio on name (handles token reordering and suffix drops)
    "tset_name",         # Token Set Ratio on name (handles acronyms, subsets, and noise)
...
def extract_pair_features(
    ...
    """
    # 1. TF-IDF Cosine Score from blocking
    ...
    # 4. Postal Code Match (1.0 = exact, 0.5 = 3-digit prefix, 0.0 = none/mismatch)
    if sp and cp:
        if sp == cp:
            f_postal = 1.0
...
def extract_features_batch(
    ...
    Args:
        cand_dict: dict {s1_id: [(candidate_id, tfidf_score), ...]}
        s1_lookup: dict {s1_id: (clean_name, address, postal)}
        cand_lookup: dict {cand_id: (clean_name, address, postal)}
        
    Returns:
        tuple: (X_matrix: np.ndarray, pair_keys: list of (s1_id, cand_id))
    """
    X_list = []
    pair_keys = []
