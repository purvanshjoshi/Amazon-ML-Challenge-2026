...
def load_filtered_candidates(tsv_paths: list, country: str, needed_ids: set) -> pd.DataFrame:
    ...
    This avoids loading multi-million row DataFrames when only a subset
    of candidates is needed (e.g., the ~500K candidates selected by blocking
    out of a 6.2M candidate pool).

    Memory usage: proportional to len(needed_ids), not total file size.
    """
    chunks = []
